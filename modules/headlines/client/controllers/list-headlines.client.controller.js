(function () {
  ...
  angular
    .module('headlines')
    .controller('HeadlinesListController', HeadlinesListController);

  HeadlinesListController.$inject = ['HeadlinesService', '$sce', '$filter', '$scope', '$state', '$meta', 'Socialshare', 'EventsService', 'Authentication'];

  function HeadlinesListController(HeadlinesService, $sce, $filter, $scope, $state, $meta, Socialshare, EventsService, Authentication) {
    var vm = this;
    ...
    HeadlinesService.query().$promise.then(function (results) {
      ...
      //vm.headlines = $sce.trustAsHtml(vm.headlines);
  }
}());
